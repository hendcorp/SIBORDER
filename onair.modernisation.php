...
<html lang="en">
  <head>
    ...
    <title>Si Border - Modernisation On Air Report</title>
...
$sql = OCIParse($connect,"SELECT A.*, NVL(B.FIMO,0) AS FIMO, NVL(B.RIMO,0) AS RIMO, NVL(B.TOTAL,0) AS TOTAL, NVL(A.TARGET - B.TOTAL,0) AS DEV, NVL(ROUND((B.TOTAL/A.TARGET)*100,0),0) AS ACH FROM
(SELECT WITEL, SUM(CASE WHEN JENIS = 'MO' AND PERIODE BETWEEN '201501' AND '2015".$Month."' THEN TARGET ELSE 0 END) AS TARGET 
FROM SB_TGT WHERE JENIS = 'MO' GROUP BY WITEL) A,
(SELECT WITEL, 
SUM(CASE WHEN PROJECT = 'FIMO' THEN 1 ELSE 0 END) AS FIMO,
SUM(CASE WHEN PROJECT = 'RIMO' THEN 1 ELSE 0 END) AS RIMO,
SUM(CASE WHEN PROJECT IN ('FIMO','RIMO') THEN 1 ELSE 0 END) AS TOTAL
FROM SB_NODE_B_MOD WHERE".$QueDate."AND STATUS_OA = 'OA' GROUP BY WITEL) B
WHERE A.WITEL = B.WITEL(+) ORDER BY ACH DESC");
ociexecute($sql);
?>
  <div class="container">
  <h3 align="center"><strong>MODERNISATION - ON AIR REPORT</strong></h3><br />
...
<tr>
    ...
    <td rowspan="2" align="center" width="35%"><br /><strong>WITEL</strong></td>
    <td rowspan="2" align="center"><br /><strong>TARGET</strong></td>
    <td colspan="3" align="center" width="30%"><strong>REAL</strong></td>
    <td rowspan="2" align="center"><br /><strong>DEV</strong></td>
    <td rowspan="2" align="center"><br /><strong>ACH %</strong></td>
  </tr>
  <tr>
    <td align="center" width="10%"><strong>FIMO</strong></td>
    <td align="center" width="10%"><strong>RIMO</strong></td> 
  ...
  <tr>
    ...
    <td align="right"><a href="detil.mo.php?witel=<?php echo $row[0]; ?>&start=<?php echo $start; ?>&end=<?php echo $end; ?>" style="text-decoration:none"><font color="#E12E32"><?php echo $row[4]; ?></font></a></td>
  ...
  <td align="right"><strong><?php echo $fimo; ?></strong></td>
  <td align="right"><strong><?php echo $rimo; ?></strong></td>
  <td align="right"><strong><a href="detil.mo.php?witel=ALL&start=<?php echo $start; ?>&end=<?php echo $end; ?>" style="text-decoration:none"><font color="#E12E32"><?php echo $total; ?></font></a></strong></td>
  ...
  <div class="row">
  <div class="col-md-8">
  <strong>GRAFIK MODERNISATION ON AIR REGIONAL 5</strong>
